...
import { FavorecidoDeleteDialog } from '../FavorecidoDeleteDialog';
...
function renderDialog(props: Parameters<typeof FavorecidoDeleteDialog>[0]) {
  return render(<FavorecidoDeleteDialog {...props} />, { wrapper: createWrapper() });
}

describe('FavorecidoDeleteDialog', () => {
  ...
  describe('default confirmation state', () => {
    it('renders confirmation message and Excluir/Cancelar buttons', () => {
      renderDialog({ isOpen: true, onClose: vi.fn(), favorecido: mockFavorecido });

      expect(
        screen.getByText('Tem certeza que deseja excluir este favorecido?')
      ).toBeInTheDocument();
    ...
    it('"Cancelar" button calls onClose without calling the delete API', async () => {
      const onClose = vi.fn();
      renderDialog({ isOpen: true, onClose, favorecido: mockFavorecido });
  ...
  describe('successful deletion', () => {
    it('calls delete API with correct id and organizationId', async () => {
      ...
      await waitFor(() => {
        expect(mockDelete).toHaveBeenCalledWith(mockFavorecido.id, ORGANIZATION_ID);
      });
    });
  ...
  describe('closed state', () => {
    it('does not render when isOpen is false', () => {
      renderDialog({ isOpen: false, onClose: vi.fn(), favorecido: mockFavorecido });
