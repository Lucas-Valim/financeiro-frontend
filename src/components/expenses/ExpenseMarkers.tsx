...
import { Repeat, FileClock, CircleAlert, type LucideIcon } from 'lucide-react';
import { cn } from '@/lib/utils';
import { EXPENSE_MARKER_COLORS } from '@/constants/expenses';
...
/**
 * Rótulos acessíveis dos três marcadores. Fonte única também para o texto: os
 * badges da lista os usam como `aria-label`, e {@link getExpenseMarkersLabel} os
...
interface MarkerDefinition {
  key: keyof typeof MARKER_LABELS;
  icon: LucideIcon;
  colorClass: string;
  testId: string;
}
...
export function getExpenseMarkersLabel(expense: ExpenseDTO): string {
  return getActiveMarkers(expense)
    .map((marker) => MARKER_LABELS[marker.key])
    .join(', ');
}
...
function Marker({
  ...
  const { icon: Icon, colorClass, testId, key } = definition;
  ...
  return (
    <span
      ...
      aria-label={isList ? MARKER_LABELS[key] : undefined}
...
/**
 * Fonte única dos três indicadores de uma despesa gerada — origem recorrente,
 * documento pendente e valor a confirmar — consumida tanto pela lista quanto
 * pelo calendário (ADR-002). A regra de exibição e o vocabulário visual vivem
 * aqui; a prop de densidade apenas ajusta a apresentação ao espaço disponível.
 */
  ...
  return (
    ...
    >
      {markers.map((definition) => (
        <Marker key={definition.key} definition={definition} density={density} />
      ))}
    </span>
  );
}
